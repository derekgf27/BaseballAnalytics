  ...
  return (
    <div className="card-tech overflow-hidden rounded-lg border">
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-x-2 gap-y-2 border-b border-[var(--border)] bg-[var(--bg-elevated)] px-3 py-2">
        ...
        <div
          className="flex items-center justify-center gap-3 text-[10px] text-[var(--text-muted)]"
          role="list"
      ...
      <div className="p-2 sm:p-3">
        <table
          ...
          role="grid"
          aria-label={`Schedule for ${monthLabel} ${year}`}
        >
          <thead>
            <tr>
              {DAYS.map((d) => (
                          ...
                          <div className="mt-0.5 flex flex-1 flex-col gap-px overflow-hidden">
                            {dayGames.slice(0, 2).map((game) => {
                              ...
                              const outcome = finalized
                                ? ourTeamOutcomeFromFinalScore(game)
                                : null;
                              ...
                              return (
                                <div
                                  ...
                                  className={`flex flex-col gap-0.5 rounded px-0.5 py-px text-left text-[10px] font-medium leading-tight break-words sm:text-[11px] ${
                                    isHome ? "text-[var(--accent)]" : "text-[var(--danger)]"
                                  }`}
                                >
                                  <Link
